...
namespace PylonSoftwareEngine.SceneManagement.Objects.Components
{
    public class CharacterController : RigidBody
    {
        ...
        public CharacterController(List<PylonSoftwareEngine.Mathematics.Triangle> Triangles, float mass = 1f) : base(Triangles, mass) { }
        ...
        /// <param name="mass">Mass</param>
        public CharacterController(PylonSoftwareEngine.Mathematics.Triangle Triangle, float mass = 1f) : base(Triangle, mass) { }
                ...
                for (int i = 0; i < SceneContext.Objects.Count; i++)
                {
                    if (SceneContext.Objects[i].Tags.Contains("Ground") && SceneContext.Objects[i] is MeshObject)
                    {
                        ...
                        if (boxobj.PointInBox(Point))
                        {
                            ...
                            {
                                meshObject.Mesh = Primitves3D.CreateCube(MySoftware.Materials.Get("DEBUG_Blue"), Min, Max - Min, Quaternion.Identity, false);
                            ...
                            foreach (var item in obj.Mesh.Points)
                            {
                                ...
                                if (PlayerFeet.PointInBox(vert))
                                {
                                    if (DebugSettings.CharacterSettings.ShowDebug)
                                    {
                                        meshObject.Mesh = Primitves3D.CreateCube(MySoftware.Materials.Get("DEBUG_Red"), Min, Max - Min, Quaternion.Identity, false);
                                        SceneContext.Add(meshObject);
                                    }
                                    return true;
                                }
